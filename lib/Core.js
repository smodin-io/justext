...
class Core {
  ...
  jusText(
    ...
    paragraphs = this.classifyParagraphs(
      paragraphs,
      stoplist,
      lengthLow,
      lengthHigh,
    ...
    paragraphs = Core.reviseParagraphClassification(
      paragraphs,
      maxHeadingDistance
    );
  ...
  classifyParagraphs(
    ...
    paragraphs.forEach((paragraph) => {
      ...
      if (Number(linksDesity) > Number(maxLinkDensity)) {
        ...
        indexOf.call(text, "\xa9") !== -1 ||
        indexOf.call(this, "&copy") !== -1
      ) {
        paragraph.cfClass = "bad";
        // TODO: update to raw regex as in python verison https://github.com/miso-belica/jusText/commit/1170debf6f5937c9a706cf850bc6e4d6db2668b8#diff-1c2d6d5086f226fb054ddd1823761a100ba31ec2ee282194f27385278ede1fb1
  ...
  static reviseParagraphClassification(paragraphs, maxHeadingDistance) {
    ...
    reviseParagraphs.forEach((paragraph, index) => {
      if (paragraph.isHeading() && paragraph.classType === "short") {
        ...
        let distance = 0;
        while (
          counter < reviseParagraphs.length &&
          distance <= maxHeadingDistance
        ) {
    ...
    reviseParagraphs.forEach((paragraph, index) => {
      if (paragraph.classType === "short") {
        const prevNeighbour = Core.getPrevNeighbour(
          index,
          reviseParagraphs,
          true
        );
        const nextNeighbour = Core.getNextNeighbour(
          index,
          reviseParagraphs,
          true
        );
        ...
        if (neighbours.length === 1 && neighbours[0] === "good") {
          ...
          (prevNeighbour === "bad" &&
            Core.getPrevNeighbour(index, reviseParagraphs, false) ===
              "neargood") ||
          (nextNeighbour === "bad" &&
            Core.getNextNeighbour(index, reviseParagraphs, false) ===
              "neargood")
        ) {
          newClassType[index] = "good";
        } else {
